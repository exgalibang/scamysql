...
public class AppUtil  {
	...
	public static boolean checkParam(String method, PageData pd){
		...
		}else if(method=="registerUser"){//app简单用户注册
			paramArray = Const.APP_REGISTERED_PARAM_ARRAY_SIMPLE;  
			valueArray = Const.APP_REGISTERED_VALUE_ARRAY_SIMPLE;
		}
		...
		if(falseCount>0){
			logger.info(method+"接口，请求协议中缺少 "+falseCount+"个 参数");
			for(int j=1;j<=falseCount;j++){
				logger.info("   第"+j+"个："+ tempArray[j-1]);
			}
		} else {
			result = true;
		}
