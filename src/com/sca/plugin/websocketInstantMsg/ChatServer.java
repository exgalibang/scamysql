package com.sca.plugin.websocketInstantMsg;
...
public class ChatServer extends WebSocketServer{
	...
	public void onMessage(WebSocket conn, String message){
		...
		if(null != message && message.startsWith("scaadmin")){
			this.userjoin(message.replaceFirst("scaadmin", ""),conn);
		}if(null != message && message.startsWith("Leavescaadmin")){
			this.userLeave(conn);
		}if(null != message && message.contains("scaadmin886")){
			String toUser = message.substring(message.indexOf("scaadmin886")+10, message.indexOf("scascaadmin888"));
			message = message.substring(0, message.indexOf("scaadmin886")) +"[私信]  "+ message.substring(message.indexOf("scascaadmin888")+12, message.length());
