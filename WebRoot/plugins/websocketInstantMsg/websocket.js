...
function toUserMsg(toU){
 ...
 function creatw() {
			...
			function initWebSocket() {
				if (window.WebSocket) {
					...
					websocket.onopen = function() {
						//连接成功
						win.setTitle(title + '&nbsp;&nbsp;(已连接)   【现在全局对话】');
						websocket.send('HMZadminqq914293814'+user);
					};
			...
			win.on("close",function(){
				websocket.send('LeaveHMZadminqq914293814');
				isCreatw = false;
			 });
			...
			function send() {
				...
				if(toUser != ""){content = "hmzadmin886"+toUser+"hmzhmzadmin888" + content;}
				var message = {};
				if (websocket != null) {
					if (input.getValue()) {
						Ext.apply(message, {
...
Ext.define('MessageContainer', {
	...
	tpl : [
			'<div class="l-im-message-warn">​欢迎使用HMZ Admin 即时通讯系统。</div>',
			'<tpl for=".">',
